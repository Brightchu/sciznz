...
class Order_model extends CI_Model {
	...
	public function userDone($userID) {
		$sql = 'SELECT `order`.`ID`, `category`.`name` AS `category`, `model`.`name` AS `model`, `device`.`ID` AS `deviceID`, `supply`.`name` AS `supply`, `supply`.`email` AS `email`, `supply`.`phone` AS `phone`, `device`.`contract` AS `contract`, `order`.`date`, `usage`.`date` AS `useDate`, `usage`.`resource`, `status`, `note`, `detail`, `budget`, `fill` FROM `order` JOIN `device` ON `userID` = ? AND (`status` = "DONE" OR `status` = "CANCEL") AND `order`.`deviceID` = `device`.`ID` JOIN `model` ON `device`.`modelID` = `model`.`ID` JOIN `category` ON `model`.`categoryID` = `category`.`ID` JOIN `supply` ON `device`.`supplyID` = `supply`.`ID` LEFT JOIN `usage` ON `order`.`usageID` = `usage`.`ID` ORDER BY `order`.`ID` DESC';
		return $this->db->query($sql, $userID)->result_array();
	}
	...
	public function supplyDone($supplyID) {
		$sql = 'SELECT `order`.`ID`, `category`.`name` AS `category`, `model`.`name` AS `model`, `device`.`ID` AS `deviceID`, `user`.`name`, `user`.`email`, `user`.`phone`, `order`.`date`, `usage`.`date` AS `useDate`, `usage`.`resource`, `status`, `note`, `detail`, `budget`, `fill` FROM `order` JOIN `device` ON `supplyID` = ? AND (`status` = "DONE" OR `status` = "CANCEL") AND `order`.`deviceID` = `device`.`ID` JOIN `model` ON `device`.`modelID` = `model`.`ID` JOIN `category` ON `model`.`categoryID` = `category`.`ID` JOIN `user` ON `order`.`userID` = `user`.`ID` LEFT JOIN `usage` ON `order`.`usageID` = `usage`.`ID` ORDER BY `order`.`ID` DESC';
		return $this->db->query($sql, $supplyID)->result_array();
	}
	...
	public function helperDone() {
		$sql = 'SELECT `order`.`ID`, `category`.`name` AS `category`, `model`.`name` AS `model`, `device`.`ID` AS `deviceID`, `user`.`name` AS `uname`, `user`.`email` AS `uemail`, `user`.`phone` AS `uphone`, `supply`.`name` AS `sname`, `supply`.`email` AS `semail`, `supply`.`phone` AS `sphone`, `device`.`contract` AS `contract`, `order`.`date`, `usage`.`date` AS `useDate`, `usage`.`resource`, `status`, `note`, `detail`, `budget`, `fill` FROM `order` JOIN `device` ON (`status` = "DONE" OR `status` = "CANCEL") AND `order`.`deviceID` = `device`.`ID` JOIN `model` ON `device`.`modelID` = `model`.`ID` JOIN `category` ON `model`.`categoryID` = `category`.`ID` JOIN `user` ON `order`.`userID` = `user`.`ID` JOIN `supply` ON `device`.`supplyID` = `supply`.`ID` LEFT JOIN `usage` ON `order`.`usageID` = `usage`.`ID` ORDER BY `order`.`ID` DESC';
		return $this->db->query($sql)->result_array();
	}

	public function groupBill($groupID) {
		$sql = 'SELECT `order`.`ID`, `userID`, `deviceID`, `order`.`date`, `status`, `detail`, `usageID`, `budgetID`, `fillID` FROM `order` JOIN `pay` ON `pay`.`method` = "GROUP" AND `pay`.`account` = ? AND (`order`.`budgetID` = `pay`.`ID` OR `order`.`fillID` = `pay`.`ID`) ORDER BY `order`.`ID` DESC';
		return $this->db->query($sql, $groupID)->result_array();
	}

	public function all() {
		$sql = 'SELECT `ID`, `userID`, `deviceID`, `date`, `status`, `detail`, `usageID`, `budgetID`, `fillID` FROM `order` ORDER BY `order`.`ID` DESC';
		return $this->db->query($sql)->result_array();
	}
